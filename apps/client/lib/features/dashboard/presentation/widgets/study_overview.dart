              ...
              child: sessionsAsync.when(
                ...
                data: (sessions) {
                  ...
                  for (final session in sessions) {
                    ...
                    // Check if session is in the current week
                    if (sessionDate.isAfter(startOfWeek.subtract(const Duration(days: 1))) && 
                      ...
                      final dayIndex = sessionDate.weekday - 1; // 0 = Mon, 6 = Sun
                      weeklyHours[dayIndex] += session.durationSeconds / 3600.0;
                    }
                  }

                  final totalHours = weeklyHours.reduce((a, b) => a + b);
                  final avgHours = totalHours / 7;
                  
                  int bestDayIndex = 0;
                  for (int i=1; i<7; i++) {
                    if (weeklyHours[i] > weeklyHours[bestDayIndex]) {
                      bestDayIndex = i;
                    }
                  }
                  ...
                  final bestH = weeklyHours[bestDayIndex].floor();
                  final bestM = ((weeklyHours[bestDayIndex] - bestH) * 60).round();
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            _StatItem('Total Time', '${totalH}h ${totalM}m', null),
                            Container(width: 1, height: 30, color: Colors.white.withOpacity(0.1)),
                            _StatItem('Avg / Day', '${avgH}h ${avgM}m', null),
                            Container(width: 1, height: 30, color: Colors.white.withOpacity(0.1)),
                            _StatItem('Best Day', days[bestDayIndex], '${bestH}h ${bestM}m', isHighlighted: true),
                          ],
                        ),
                      ),
                    ],
